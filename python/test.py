...
def plot_surface_and_spectrum(random_field, file_name="RandomField.png"):
    ...
    ax[0].set_title("Random Field (2D), $z/\sigma$")
    ax[0].set_aspect('equal')
    ax[0].set_xticks([])
    ax[0].set_yticks([])
    ax[0].set_xlabel("$x$")
    ax[0].set_ylabel("$y$")
    plt.colorbar(im, ax=ax[0], orientation='horizontal', shrink=colorbar_shrink, pad=0.05, label='Field Value, $z/\sigma$')
    ...
    ax[1].set_title("Power Spectrum (log scale), $\Phi(k_x, k_y)$")
    plt.colorbar(im, ax=ax[1], orientation='horizontal', shrink=colorbar_shrink, pad=0.05, label='Power Spectrum, $log_{10}(\Phi)$')
...
def main():
    ...
    k_low = 16 / N      # Lower cutoff of the power spectrum
    ...
    random_field = rf.periodic_gaussian_random_field(dim = dim, N = N, Hurst = Hurst, k_low = k_low, k_high = k_high)
    ...
    ideal_random_field = rf.ideal_periodic_gaussian_random_field(dim = dim, N = N, Hurst = Hurst, k_low = k_low, k_high = k_high)
